...
function setChartDataA(gridViewPointer, curChartModelSelector, chart, chartID) {
  var rtnDataName = gridViewPointer.pageID+'.'+chartID;
  var dataNumb = 0;
  var nonRealTimeChartData = [];
  var RealTimeChartData = [];
  ...
  userApi.emitter.on (rtnDataName, function(data){
    ...
    if (true === isRealTime) {      
      RealTimeChartData.push(curRtnData); 
      realDateTime.push(data.MonTime);

      // if (chartDataAll.length < 10) {
        console.log ('**** real data.MonTime: ' + data.MonTime);
        // console.log ('chartDataAll.length: ' + chartDataAll.length);
        // console.log ('testChartDataNumb: ' + testChartDataNumb);
      ...
      //      MinusTime(curTime, firstTime) > timeLimit)  {      
        if (dataNumb > 0) {
          ...
          // console.log ('CompareArray : ' + compareArray(chartDataAll, nonRealTimeChartData));

          // chart.series[0].setData(nonRealTimeChartData); 
          curChartData = [];
          for (var i = 0; i < testData.length; ++i) {
            curChartData.push(testData[i]);
          }
          ...
          console.log ('chartDataAll.length: ' + chartDataAll.length);
          // console.log (chartDataAll[chartDataAll.length-1]);
    ...
    console.log ('curChartData.length: ' + curChartData.length);
    console.log ('CompareArray : ' + compareArray(curChartData, testData));
  ...
  //     } else {
  //       for (var i = 0; i < RealTimeChartData.length; ++i) {
  //         console.log ('Add, data.MonTime: ' + realDateTime[i]);
  //         addDataToChart(chart, RealTimeChartData[i]);          
  //       }
  //       RealTimeChartData = [];
  //       realDateTime = [];
  ...
  userApi.emitter.on (rtnDataName, function(data){
    ...
    if (true === isRealTime) {
      ...
      if (true === curActive) {
        ...
        // console.log ('curChartData.length: ' + curChartData.length);
        // console.log ('testData.length: ' + testData.length);
        // console.log ('CompareArray : ' + compareArray(chartDataAll, testData));
      }
...
function setChartDataC(gridViewPointer, curChartModelSelector, chart, chartID) {
  var rtnDataName = gridViewPointer.pageID+'.'+chartID;
  var dataNumb = 0;
  ...
  userApi.emitter.on (rtnDataName, function(data){
    ...
    // if (true === isItemTurnToActive) {
    //   chart.series[0].setData(chartDataAll);
    //   return;
    // }
    if (true !== curActive) {
      console.log ('hide!');
  ...
  setInterval(function() {
    ...
    if (true === isNewDataCome && true === curActive) {
      ...
      //   chart.series[0].setData(nonRealTimeChartData);
      // } else {
      // // } else if (true !== isItemTurnToActive){
      //   for (var i = 0; i < RealTimeChartData.length; ++i) {
      //     console.log ('add : ' + i);          
      //     addDataToChart(chart, RealTimeChartData[i]);          
      //   }
