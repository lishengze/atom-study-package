...
function draw(chartID) {
  if (!chartObj[chartID]) {
    chartObj[chartID]=new Object();
    chartObj[chartID].svg=null;
    chartObj[chartID].yAxisGroup=null;
    chartObj[chartID].xAxisGroup=null;
    chartObj[chartID].dataCirclesGroup=null;
    chartObj[chartID].dataLinesGroup=null;
  }
	...
	chartObj[chartID].svg = d3.select(chartID).select('svg').select('g');
	if (chartObj[chartID].svg.empty()) {
		chartObj[chartID].svg = d3.select(chartID)
			.append('svg:svg')
				.attr('width', w)
				.attr('height', h)
				.attr('class', 'viz')
	...
	t = chartObj[chartID].svg.transition().duration(transitionDuration);

	// y ticks and labels
	if (!chartObj[chartID].yAxisGroup) {
		chartObj[chartID].yAxisGroup = chartObj[chartID].svg.append('svg:g')
			.attr('class', 'yTick')
			.call(yAxis);
	}
	else {
		t.select(chartID+' .yTick').call(yAxis);
	}

	// x ticks and labels
	if (!chartObj[chartID].xAxisGroup) {
		chartObj[chartID].xAxisGroup = chartObj[chartID].svg.append('svg:g')
			.attr('class', 'xTick')
			.call(xAxis);
	}
	else {
		t.select(chartID+' .xTick').call(xAxis);
	}

	// Draw the lines
	if (!chartObj[chartID].dataLinesGroup) {
		chartObj[chartID].dataLinesGroup = chartObj[chartID].svg.append('svg:g');
	}

	var dataLines = chartObj[chartID].dataLinesGroup.selectAll('.data-line')
			.data([data]);
  ...
  //           	.attr("d", garea(data));
	...
	// // Draw the points
	// if (!chartObj[chartID].dataCirclesGroup) {
	// 	chartObj[chartID].dataCirclesGroup = chartObj[chartID].svg.append('svg:g');
	// }
  //
	// var circles = chartObj[chartID].dataCirclesGroup.selectAll('.data-point')
	// 	.data(data);
  //
	// circles
	// 	.enter()
...
var beginReceiveData=function(){
  ...
  var url = 'http://192.168.1.108:80';
  rootSocket = io.connect(url);
  rootSocket.on('new data comes', function(data){
        // var newData=randomData1(data);
  			// change1(newData);
        change1(randomData1(data));
  		});
  rootSocket.emit('start data stream', 'start');
}

var stopReceiveData=function(){
  rootSocket.emit('stop emit data', " ");
}
